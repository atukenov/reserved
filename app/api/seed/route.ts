...
export const GET = async () => {
  ...
  const indianCuisineId =
    cuisines.find((cuisine) => cuisine.name === "indian")?._id || 1;
  const mexicanCuisineId =
    cuisines.find((cuisine) => cuisine.name === "mexican")?._id || 1;
  const italianCuisineId =
    cuisines.find((cuisine) => cuisine.name === "italian")?._id || 1;

  const ottawaLocationId =
    locations.find((location) => location.name === "ottawa")?._id || 1;
  const torontoLocationId =
    locations.find((location) => location.name === "toronto")?._id || 1;
  const niagaraLocationId =
    locations.find((location) => location.name === "niagara")?._id || 1;
  ...
  const curryishTavernId =
    restaurants.find((restaurant) => restaurant.name === "Curryish Tavern")
      ?._id || 1;
  const utsavId =
    restaurants.find((restaurant) => restaurant.name === "Utsav")?._id || 1;
  const pukkaId =
    restaurants.find((restaurant) => restaurant.name === "Pukka")?._id || 1;
  ...
  const eldoradoTacoId =
    restaurants.find((restaurant) => restaurant.name === "Eldorado Taco")
      ?._id || 1;
  const laBartolaId =
    restaurants.find((restaurant) => restaurant.name === "La Bartola")?._id ||
    1;
  const elCatrinId =
    restaurants.find((restaurant) => restaurant.name === "El Catrin")?._id || 1;
  const mariachisId =
    restaurants.find((restaurant) => restaurant.name === "3 Mariachis")?._id ||
    1;
  ...
  const bluRistoranteId =
    restaurants.find((restaurant) => restaurant.name === "Blu Ristorante")
      ?._id || 1;
  const stelvioId =
    restaurants.find((restaurant) => restaurant.name === "Stelvio")?._id || 1;
  const sofiaId =
    restaurants.find((restaurant) => restaurant.name === "Sofia")?._id || 1;
  ...
  const userJosh = await User.create({
    firstName: "Josh",
    lastName: "Allen",
  ...
  const userLebron = await User.create({
    firstName: "LeBron",
    lastName: "James",
  ...
  const userCassidy = await User.create({
    firstName: "Cassidy",
    lastName: "Marksom",
  ...
  newReviews.forEach(async (review) => {
    ...
    restaurant.reviews.push(added._id);
    await restaurant.save();
  });
